...
import com.android.testing.other.Event
...
class ShoppingViewModel @Inject constructor(
    ...
    fun insertShoppingItem(name: String, amountString: String, priceString: String) {
        if (name.isEmpty() || amountString.isEmpty() || priceString.isEmpty()) {
            _insertShoppingItemStatus.postValue(Event(Resource.error("Field must not be empty.", null)))
            return
        }

        if (name.length > Constants.MAX_NAME_LENGTH) {
            _insertShoppingItemStatus.postValue(Event(Resource.error("The name of the item must not exceed" +
                    "${Constants.MAX_PRICE_LENGTH}.", null)))
            return
        }
